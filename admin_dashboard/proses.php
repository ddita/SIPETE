...
<body>
  ...
  if(isset($con, $_POST['tambahsaldo'])) {
    ...
    if(mysqli_num_rows($pgl_saldoakhir) > 0) {
      ...
      echo '<script>alert("Saldo berhasil ditambahkan."); window.location.href = "../admin_dashboard/index.php";</script>';
      
    } else {
      echo '<script>alert("Kode bank tidak ditemukan."); window.location.href = "../admin_dashboard/index.php";</script>';
    }

  } else {
    // Redirect jika tidak ada submit form
    echo '<script>window.location = "../admin_dashboard/index.php"</script>';
  }
